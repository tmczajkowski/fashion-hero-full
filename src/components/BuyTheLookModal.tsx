...
export interface BuyTheLookProduct {
  id: string;
  name: string;
  price: string;
  image: string;
...
function QuickViewModal({ product, onClose, onAddToCart }: QuickViewModalProps) {
  ...
  return (
    <div
      className="fixed inset-0 z-[60] flex items-start md:items-center justify-center bg-black/50"
      onClick={onClose}
    >
      <div
...
interface BuyTheLookModalProps {
  isOpen: boolean;
  onClose: () => void;
  productName: string;
  productImage: string;
...
export function BuyTheLookModal({
  isOpen,
  onClose,
  productName,
  productImage,
                    ...
                    <div className="p-3 space-y-3">
                      ...
                      <button
                        onClick={() => {
                          ...
                          posthog.capture("buy_the_look_popup_add_to_bag");
                        }}
